...
// Helpers
//-----------------------------------------------------------------------------
const htmlTagPattern = /<([a-z0-9]+(?:-[a-z0-9]+)*)(?:\s+(?:[^>"']|"[^"]*"|'[^']*')*)?>/giu;
...
export default {
    ...
    create(context) {
        ...
        const allowedElements = new Set(allowedIgnoreCase ? allowed.map(tag => tag.toLowerCase()) : allowed);
        return {
            html(node) {
                let match;
                while ((match = htmlTagPattern.exec(node.value)) !== null) {
                    ...
                    const endColumn = firstNewlineIndex === -1
