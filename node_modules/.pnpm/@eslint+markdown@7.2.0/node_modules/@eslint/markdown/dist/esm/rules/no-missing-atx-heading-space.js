...
function findMissingSpaceBeforeClosingHash(text) {
    ...
    if (match) {
        const [, closingSequenceSpaces, closingSequence, trailingSpaces] = match;
        if (closingSequenceSpaces.length === 0) {
            const closingHashIdx = text.length - (trailingSpaces.length + closingSequence.length);
                ...
                if (missingSpace) {
                    context.report({
                        loc: {
                            start: {
                                line: lineNum,
                                column: startColumn + missingSpace.beforeHashIdx,
                            },
                            end: {
                                line: lineNum,
